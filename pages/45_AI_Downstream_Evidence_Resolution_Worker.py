merge acum fisieru avea 2 in paranteza si lam sters
Fri, Jul 24 at 10:52 AM
GrantAI_Europe_STAGE8_1_FIXED(1).zip
Zip Archive
am inlocuit si apare asta 
GrantAI_Europe_STAGE8_1_FIXED(2).zip
Zip Archive
13_AI_Proposal_Reviewer(1).py
Python
12_AI_Grant_Writer(1).py
Python
13_AI_Proposal_Reviewer (1).py
Python
14_AI_Proposal_Optimizer (1).py
Python

Am identificat din capturi că Etapa 16 inițializează Supabase prin create_client(...), folosind st.secrets / variabile de mediu, nu prin st.session_state["supabase"]. De aceea Etapa 17 afișa „Conexiunea Supabase nu este disponibilă”.

Am corectat toată Etapa 17, nu doar conexiunea, pentru că mai erau trei incompatibilități importante: codul vechi căuta proposal_reviews în loc de grant_evaluations, proposal_sections în loc de grant_writer_sections și încerca să lege readiness items prin run_id, coloană care nu există în schema Etapei 16.

Descarcă varianta corectată 17_AI_Submission_Pack_FIXED.py

În GitHub deschide pages/17_AI_Submission_Pack.py, șterge tot conținutul din acel fișier și înlocuiește-l cu conținutul fișierului corectat de mai sus. Numele din GitHub trebuie să rămână:

17_AI_Submission_Pack.py

Apoi fă Commit changes, așteaptă redeploy și intră din nou în AI Submission Pack. Nu trebuie să modifici nimic în Supabase. După ce se deschide, trimite-mi captura. 

17_AI_Submission_Pack_FIXED.py
Code
Pasted text(1).txt
Document
desi am intrudus numele si parola apare asta
apare acelas lucru
20_AI_Resolution_Executor(1).py
Python
19_AI_Resolution_Agent(1).py
Python
20_AI_Resolution_Executor(2).py
Python
19_AI_Resolution_Agent(2).py
Python
20_AI_Resolution_Executor(3).py
Python
21_AI_Evidence_Input_Manager(1).py
Python
22_AI_Evidence_Autofill(1).py
Python
merge
Pasted text(2).txt
Document
...
Pasted text(6).txt
Document
36_AI_Official_Opportunity_Verification_Selection_Gate(1).py
Python
36_AI_Official_Opportunity_Verification_Selection_Gate (1).py
Python
36_AI_Official_Opportunity_Verification_Selection_Gate (2).py
Python
merge
37_AI_Selected_Opportunity_Lock_Workflow_Handoff(1).py
Python
acuma merge 38
38_AI_Locked_Opportunity_Evidence_Requirements_Bootstrap(1).py
Python
45_AI_Downstream_Evidence_Resolution_Worker(1).py
Python
45_AI_Downstream_Evidence_Resolution_Worker (1).py
Python
45_AI_Downstream_Evidence_Resolution_Worker (2).py
Python

Gata. Am făcut Stage 45 v7.10 complet, pornind din v7.9.1 și păstrând resolverul existent.

v7.10 adaugă stratul Topic Document & Annex Resolver pentru:

identificarea documentului oficial al topicului exact;
clasificarea TOPIC_PAGE / PDF / ANNEX / WORK_PROGRAMME / CALL_DOCUMENT;
urmărirea provenance chain;
căutarea separată pentru Applicant eligibility, Consortium requirements și TRL requirements;
RESOLVED numai când există pasaj oficial explicit și trasabil;
păstrarea resolution_method = OFFICIAL_DOCUMENTATION, deci nu reintroducem eroarea SQL 23514.

Am validat și sintaxa Python.

Descarcă Stage 45 v7.10 complet

Înlocuiește v7.9.1 cu acesta și rulează Stage 45. După rezultat, trimite captura cu Stage 45 v7.10 — Topic Document & Annex Diagnostics. 

45_AI_Downstream_Evidence_Resolution_Worker_v7_10.py
Code
Today 11:25 AM

Da. Acum problema este clară și este în codul v7.10 pe care l-am făcut:

opportunity_lock_id nu este definit în scope-ul panoului nou.
opportunity_identity nu este definit în același scope.
De aceea v7.10 ajunge la FAILED, înainte ca diagnosticul nou să poată lucra.
Reference graph nodes = 50 arată că partea veche continuă să găsească graful; problema actuală este integrarea variabilelor, nu resolverul în sine.

Trebuie să leg v7.10 de variabilele reale folosite deja de v7.9.1, nu să presupun numele lor.

Pot să refac acum v7.10.1 complet, direct din fișierul v7.10 pe care l-am generat, cu rezolvarea sigură a lock_id și opportunity_identity, astfel încât să nu mai producă NameError.

continua cu v7.10.1

Gata. Am făcut Stage 45 v7.10.1 complet și am reparat exact cele două NameError:

opportunity_lock_id → lock_id
opportunity_identity → identity

Am actualizat și etichetele la v7.10.1 și am validat sintaxa Python.

Descarcă Stage 45 v7.10.1

Înlocuiește v7.10 cu acesta și rulează din nou. Panoul Stage 45 v7.10.1 — Topic Document & Annex Diagnostics ar trebui acum să se încarce fără cele două erori de variabile. 

45_AI_Downstream_Evidence_Resolution_Worker_v7_10_1.py
Code
...
lock = locks[0]
lock_id = str(lock["id"])
identity = normalize_text(lock.get("identity"))
...
def s45v79_run_task(task, worker_run_id, graph, fetch_cache):
    inserted = (supabase.table("locked_evidence_worker_items").insert({
        "user_id": user_id, "project_id": project_id, "lock_id": lock_id,
        ...
        "identity": identity, "requirement_key": task.get("requirement_key"),
    ...
    return 
# ============================================================
# STAGE 45 v7.10.1 — TOPIC DOCUMENT & ANNEX RESOLVER
# ============================================================
...
st.divider()
st.subheader("Stage 45 v7.10.1 — Topic Document & Annex Resolver")
st.info(
    "v7.10.1 folosește resolution_method canonic OFFICIAL_DOCUMENTATION și pornește de la documentul oficial al topicului blocat, urmărește recursiv referințele oficiale până la 3 niveluri, "
...
try:
    ...
    st.warning(f"v7.10.1 topic document + annex graph warning: {type(exc).__name__}: {str(exc)[:500]}")
...
with st.expander("v7.10.1 topic document + annex graph", expanded=False):
    if v79_graph:
...
if v79_tasks and st.button("🧬 Run Stage 45 v7.10.1 reference-chain resolution", type="primary", use_container_width=True, key="stage45_v79_run"):
    run = (supabase.table("locked_evidence_worker_runs").insert({
        "user_id": user_id, "project_id": project_id, "lock_id": lock_id,
        "execution_run_id": execution_run_id, "identity": identity,
    ...
    if not run:
        ...
        run_items = rows("locked_evidence_worker_items", {"user_id": user_id, "project_id": project_id, "lock_id": lock_id, "worker_run_id": run_id}, "created_at", 100)
...
v79_runs = rows("locked_evidence_worker_runs", {"user_id": user_id, "project_id": project_id, "lock_id": lock_id}, "created_at", 50)
v79_runs = [r for r in v79_runs if normalize_text(r.get("deep_resolution_version")).lower() in {"v7.9", "v7.10.1"}]
if v79_runs:
    latest = v79_runs[0]
    ...
    latest_items = rows("locked_evidence_worker_items", {"user_id": user_id, "project_id": project_id, "lock_id": lock_id, "worker_run_id": str(latest.get("id"))}, "created_at", 100)
    if latest_items:
        st.dataframe([{
...
st.caption("v7.10.1 invariant: exact-topic identity plus a traceable official document chain establishes applicability; COMPLETED still requires an explicit authoritative passage. COMPLETED still requires an explicit authoritative passage that answers the locked requirement.")
# =====================================================================
# END STAGE 45 v7.9
# =====================================================================
...
# ============================================================
# v7.10.1 TOPIC DOCUMENT + ANNEX DIAGNOSTICS
# ============================================================
try:
    st.divider()
    ...
    try:
        ...
        st.warning(f"v7.10.1 document diagnostic unavailable: {_v710_exc}")
    ...
    st.warning(f"Stage 45 v7.10.1 diagnostics could not be rendered: {_v710_ui_exc}")
