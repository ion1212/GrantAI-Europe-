...
st.set_page_config(
    page_title="Stage 71 v1.1 — Final Portal Submission Readiness",
    page_icon="✅",
    layout="wide",
)
...
if stage70:
    ...
    application_reference = normalize_text(stage70.get("application_reference"))
    draft_title = normalize_text(stage70.get("draft_title"))
    current_portal_url = normalize_text(stage70.get("current_portal_url"))
...
stage69 = get_bound("stage69_portal_upload_confirmation_runs", stage69_run_id)
stage68 = get_bound("stage68_upload_package_preparation_runs", stage68_run_id)
...
if not existing_stage71:
    ...
    readiness_payload = {
        "readiness_version": "stage71-v1.1",
        "user_id": user_id,
        "project_id": project_id,
        "opportunity_lock_id": lock_id,
    ...
    run_basis = {
        "stage": 71,
        "fingerprint_contract": "stage71-v1.1-final-submission-readiness",
        "user_id": user_id,
        "project_id": project_id,
        "opportunity_lock_id": lock_id,
        "stage70_run_id": stage70_run_id,
...
st.caption(
    "Invariantă Stage 71 v1.1: READY_FOR_SUBMISSION_AUTHORIZATION is a readiness state only. "
