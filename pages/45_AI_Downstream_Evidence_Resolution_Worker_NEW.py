...
# =====================================================================
# STAGE 45 v7.10.8 — DIAGNOSTIC FETCH + OFFICIAL TOPIC DOCUMENT DISCOVERY
# =====================================================================
...
def s45v7107_fetch(url, timeout=45):
    ...
    try:
        r = requests.get(
            ...
            allow_redirects=True,
            headers={
                'User-Agent': 'Mozilla/5.0 GreenRise/Stage45-v7.10.8',
...
def s45v7107_discover_and_verify(task):
    ...
    if not requirement_key:
        return {'status':'WAITING_OFFICIAL','reason':'Requirement family is not handled by v7.10.8.','attempts':[]}
    ...
    while queue and len(visited)<max_fetches:
        ...
        if exact and excerpt:
            return {'status':'RESOLVED','requirement_key':requirement_key,'exact_topic_verified':True,
                    ...
                    'reason':'v7.10.8 resolved from the bounded exact-topic section of a freshly fetched official EC work programme.','attempts':attempts}
        ...
        if requirement_key=='TRL_REQUIREMENTS':
            ...
            if neg:
                ...
                return {'status':'RESOLVED','requirement_key':requirement_key,'exact_topic_verified':True,
                        ...
                        'reason':'v7.10.8 parsed the complete bounded exact-topic section and found no topic-specific TRL condition; no TRL value was inferred. Stage 46 must independently verify this negative finding.','attempts':attempts}
        if bridge and annex_doc and requirement_key in ('APPLICANT_ELIGIBILITY','CONSORTIUM_REQUIREMENTS'):
            ...
            if annex_excerpt:
                ...
                return {'status':'RESOLVED','requirement_key':requirement_key,'exact_topic_verified':True,
                        ...
                        'reason':'v7.10.8 proved applicability through the exact-topic work programme delegation to General Annex B, then extracted the requirement from the freshly fetched official General Annexes.','attempts':attempts}
...
def s45v7107_save_verified_document(task, result):
    ...
    row = {
        ...
        'applicability_reason': 'v7.10.8 strict canonical exact-topic verification passed.',
        'evidence_found': True,
...
def s45v7107_persist_worker_item(task, worker_run_id, result, source_row):
    ...
    if not saved:
        raise RuntimeError('v7.10.8 could not persist Stage 46 handoff worker item.')
    return saved[0]
...
st.divider()
st.subheader('Stage 45 v7.10.8 — PDF Fix + Official Topic Document Discovery')
st.info(
    'v7.10.8 pornește numai din lock-ul canonic și discovery exact-topic, nu reciclează cele 128 de '
...
if _v7107_seed_urls:
    ...
    st.warning('Nu există canonical seed valid în ACTIVE lock; v7.10.8 va rămâne fail-closed.')

if st.button(
    '🧪 Run Stage 45 v7.10.8 PDF-fix diagnostic',
    type='primary',
    use_container_width=True,
    key='stage45_v7108_run',
):
    _run = (
        supabase.table('locked_evidence_worker_runs')
        .insert({
            ...
            'worker_status': 'RUNNING',
            'deep_resolution_version': 'v7.10.8',
            'diagnostic_status': 'CLEAN',
            'error_count': 0,
            'started_at': now_iso(),
            'summary': {
    ...
    if not _run:
        st.error('Nu am putut crea Stage 45 v7.10.8 run.')
    else:
        ...
        st.success(
            f'Stage 45 v7.10.8: {_final} — canonical RESOLVED {_resolved}, '
...
_v7107_runs = [
    ...
    if normalize_text(r.get('deep_resolution_version')).lower() == 'v7.10.8'
]

if _v7107_runs:
    _latest_v7107 = _v7107_runs[0]
    st.subheader('Latest Stage 45 v7.10.8 Result')
...
st.caption(
    'v7.10.8 invariant: Search API, CAS/EU Login, auth/error pages, static assets and historical '
...
def s45v7109_is_canonical_official_pdf(url):
    url = normalize_text(url)
    if not url:
        return False
    ...
    try:
        ...
        if any(token in full for token in (
            "search-api",
            "/cas/",
            "cas?",
            "/login",
...
def s45v7109_verify_final_pdf(url):
    """
    Fresh independent transport check using the proven v7.10.8 fetch/parser.
    No verdict is manufactured here.
    """
    fetched = s45v7107_fetch(url, timeout=45)
    ...
    result = {
        ...
        "content_type": normalize_text(fetched.get("content_type")),
        "bytes_received": fetched.get("bytes_received") or 0,
        "parser": normalize_text(fetched.get("parser")),
        "text_chars": fetched.get("text_chars") or 0,
        "text": normalize_text(fetched.get("text")),
        "error": normalize_text(fetched.get("error")),
        "rejected_reason": normalize_text(fetched.get("rejected_reason")),
    }

    if not fetched.get("ok"):
        return result

    if not s45v7109_is_canonical_official_pdf(result["final_url"]):
        result["rejected_reason"] = "FINAL_URL_NOT_CANONICAL_OFFICIAL_PDF"
        return result

    if not result["text"]:
        result["rejected_reason"] = "EMPTY_FINAL_PDF_TEXT"
        return result
...
def s45v7109_requirement_key(item):
    return normalize_text(item.get("requirement_key")).upper()


def s45v7109_reproduce_evidence(item, fetched):
    """
    Reproduce exact-topic + explicit requirement from the final PDF.
    Uses the existing v7.10.8 strict extractors.
    """
    text = normalize_text(fetched.get("text"))
    final_url = normalize_text(fetched.get("final_url"))
    requirement_key = s45v7109_requirement_key(item)
    ...
    exact_topic = s45v7107_exact_topic_in_substantive_source({
        ...
        "title": normalize_text(item.get("source_title")),
    })

    section = s45v7107_topic_section(text) if exact_topic else ""
    ...
    if (
        ...
        and exact_topic
        and not normalize_text(excerpt)
    ):
        excerpt = s45v7107_negative_trl_evidence(section or text)

    return {
        "exact_topic": bool(exact_topic),
        "explicit_evidence": bool(normalize_text(excerpt)),
        "excerpt": normalize_text(excerpt),
        "final_url": final_url,
    }


def s45v7109_update_execution_candidate(item, final_url, excerpt):
    task_id = item.get("execution_task_id")
    if not task_id:
        return
    ...
    candidate.update({
        ...
        "evidence_excerpt": excerpt[:12000],
        "exact_topic_verified": True,
        "authoritative_source_verified": True,
        "explicit_evidence_verified": True,
        "strict_canonical_source_verified": True,
...
def s45v7109_normalize_item(item):
    old_url = normalize_text(item.get("evidence_url"))

    out = {
        "Requirement": item.get("requirement_label"),
        "Worker item": item.get("id"),
        "Status": "REJECTED",
        "Old URL": old_url,
        "Canonical PDF URL": "",
        "HTTP": None,
    ...
    if not old_url:
        out["Reason"] = "MISSING_EVIDENCE_URL"
        return out

    fetched = s45v7109_verify_final_pdf(old_url)

    out["Canonical PDF URL"] = normalize_text(fetched.get("final_url"))
    out["HTTP"] = fetched.get("http_status")
    out["Type"] = fetched.get("content_type")
    out["Parser"] = fetched.get("parser")
    out["Chars"] = fetched.get("text_chars") or 0

    if not fetched.get("ok"):
        out["Reason"] = (
            fetched.get("rejected_reason")
            or fetched.get("error")
            or "FINAL_PDF_TRANSPORT_FAILED"
        )
        return out

    proof = s45v7109_reproduce_evidence(item, fetched)
    out["Exact topic"] = proof["exact_topic"]
    out["Explicit evidence"] = proof["explicit_evidence"]

    if not proof["exact_topic"]:
        out["Reason"] = "EXACT_TOPIC_NOT_REPRODUCED_FROM_FINAL_PDF"
        return out

    if not proof["explicit_evidence"]:
    ...
    if old_url and old_url not in provenance:
        provenance.append(old_url)
    if final_url and final_url not in provenance:
    ...
    s45v7109_update_execution_candidate(
        item,
        final_url,
        excerpt,
    )
    ...
    out["Reason"] = (
        "Fresh final official PDF reproduced exact topic and explicit "
        "requirement. Stage 46 must independently re-fetch it."
    )
    return out
...
st.divider()
st.subheader(
    "Stage 45 v7.10.9 — Canonical Final-PDF Handoff Fix (selector + DB fixed)"
)

st.info(
    "v7.10.9 repară numai transportul/provenance pentru handoff. "
    "Acceptă exclusiv PDF oficial EC real după redirect, reproduce din nou "
    "topic-ul exact și pasajul explicit, apoi persistă FINAL URL. "
    "Nu acordă PASS în Stage 46."
)
...
if st.button(
    "🔗 Run Stage 45 v7.10.9 canonical PDF handoff fix",
    type="primary",
    use_container_width=True,
    ...
    for _idx, _item in enumerate(_v7109_candidates, 1):
        try:
            _result = s45v7109_normalize_item(_item)
        except Exception as _exc:
            _result = {
                "Requirement": _item.get("requirement_label"),
                "Worker item": _item.get("id"),
                "Status": "ERROR",
                "Old URL": _item.get("evidence_url"),
                "Canonical PDF URL": "",
                "HTTP": None,
                "Type": "",
                "Parser": "",
        ...
        _v7109_results.append(_result)

        if _v7109_candidates:
            _bar.progress(_idx / len(_v7109_candidates))

    st.session_state["stage45_v7109_results"] = _v7109_results
    st.rerun()
...
if _v7109_results:
    st.subheader("Latest Stage 45 v7.10.9 Result")
...
st.caption(
    "v7.10.9 invariant: Stage 45 may normalize a freshly verified final "
    "official PDF URL, but Stage 46 must independently GET that PDF and "
    "reproduce exact-topic applicability plus explicit evidence."
)
...
# =====================================================================
# STAGE 45 v7.10.11 — GENERAL ANNEX + GEOGRAPHIC PROVENANCE RESOLVER
# =====================================================================
# Purpose:
#   * preserve TRL evidence already resolved by v7.10.10;
#   * resolve Applicant, Consortium and Geographic requirements only from
#     freshly fetched authoritative EC documents;
#   * require an exact-topic applicability bridge before using General Annexes;
#   * persist a complete provenance_chain for Stage 46 independent re-fetch;
#   * remain fail-closed: no evidence => WAITING_OFFICIAL.

def s45v71011_requirement_key(task):
    return s45v7107_requirement_key(task)


def s45v71011_gate_task(task):
    return s45v71011_requirement_key(task) in {
        "APPLICANT_ELIGIBILITY",
        "CONSORTIUM_REQUIREMENTS",
...
def s45v71011_general_annex_requirement(task_key):
    return task_key in {
        "APPLICANT_ELIGIBILITY",
        "CONSORTIUM_REQUIREMENTS",
        "GEOGRAPHIC_ELIGIBILITY",
...
def s45v71011_annex_excerpt(text, requirement_key):
    """
    Extract only an explicit General Annex rule.  No inference.
    """
    body = re.sub(r"\s+", " ", normalize_text(text))
    if not body:
        return ""
...
def s45v71011_exact_topic_applicability_bridge(cluster_text):
    """
    Prove that the exact topic points to general eligibility/admissibility rules.
    Requires an explicit Annex-B / General Annexes reference in the bounded
    exact-topic section.
    """
    section = s45v7107_topic_section(cluster_text)
    if not section:
        return {"ok": False, "section": "", "excerpt": ""}
    ...
    annex_patterns = (
        ...
        r"general\s+annex\s+b\b",
        r"\bannex\s+b\b",
        r"general\s+annexes\b",
    )
    annex_ref = any(re.search(p, low, flags=re.I) for p in annex_patterns)
    ...
    applicability = any(term in low for term in applicability_terms)

    if not (annex_ref and applicability):
        return {"ok": False, "section": section, "excerpt": ""}

    marker_positions = []
    for token in ("general annex", "annex b", "eligibility", "admissibility", "general conditions"):
        pos = low.find(token)
        if pos >= 0:
    ...
    if marker_positions:
        ...
        excerpt = section[max(0, idx - 1200): min(len(section), idx + 4500)].strip()
    else:
        excerpt = section[:5500].strip()
...
def s45v71011_is_general_annex_url(url):
    low = normalize_text(url).lower()
    if not low:
        return False
    return (
        ...
        or "general%20annex" in low
        or ("annex" in low and low.endswith(".pdf"))
    )


def s45v71011_collect_candidate_urls(seed_fetches):
    """
    Collect only authoritative, document-like URLs from already fetched official
    payloads/pages. Search API remains discovery-only.
    """
    out = []
    for fetched in seed_fetches:
        if not isinstance(fetched, dict):
            continue

        for u in s45v7107_discover_links_from_substantive_source(fetched):
            if u not in out:
                out.append(u)

        payload = fetched.get("json")
        if isinstance(payload, (dict, list)):
            for u in s45v7107_urls_from_exact_search_object(payload):
                if (
                    u
                    and s45v7107_allowed_official_host(u)
                    and not s45v7107_bad_url(u)
                    and u not in out
                ):
                    out.append(u)
...
def s45v71011_discover_and_verify(task):
    task_key = s45v71011_requirement_key(task)
    if task_key not in {
        "APPLICANT_ELIGIBILITY",
        "CONSORTIUM_REQUIREMENTS",
        "GEOGRAPHIC_ELIGIBILITY",
        ...
        return {
            "status": "WAITING_OFFICIAL",
            "requirement_key": task_key,
            "reason": "Requirement family is not handled by v7.10.11.",
            "attempts": [],
        }

    # Preserve the already working strict TRL resolver.
    if task_key == "TRL_REQUIREMENTS":
        return s45v7107_discover_and_verify(task)

    queue = []
    for u in (
        s45v7107_url_from_lock_context()
        + s45v7107_authoritative_reference_urls()
        + s45v7107_search_urls()
    ):
        if u and u not in queue:
            queue.append(u)
    ...
    while queue and len(visited) < max_fetches:
        ...
        final_url = normalize_text(fetched.get("final_url"))
        discovery_only = (
            s45v7107_is_search_api(final_url)
            or s45v7107_is_search_api(url)
        )
        ...
        audit = {
            ...
            "search_api_discovery_only": discovery_only,
            "exact_topic": False,
            "explicit_evidence": False,
        ...
        if discovery_only:
            for d in s45v7107_discover_from_search_api(fetched):
                if d not in visited and d not in queue:
                    queue.append(d)
            attempts.append(audit)
            continue
        ...
        if exact and (
            "wp-" in lowurl
            or "work-programme" in lowurl
            or "horizon-2026-2027" in lowurl
            or "horizon-europe-work-programme" in lowurl
        ):
            cluster_doc = fetched

        if s45v71011_is_general_annex_url(final_url):
            annex_docs.append(fetched)

        # Discovery from substantive official source.
        for d in s45v7107_discover_links_from_substantive_source(fetched):
            if d not in visited and d not in queue:
                queue.append(d)
    ...
    # If we have the exact-topic document, inspect it for the explicit
    # General-Annex applicability bridge.
    if not cluster_doc:
        return {
            "status": "WAITING_OFFICIAL",
            "requirement_key": task_key,
            ...
            "evidence_url": None,
            "evidence_excerpt": None,
            "reason": "Exact-topic Work Programme document was not established.",
            "attempts": attempts,
        }

    cluster_text = normalize_text(cluster_doc.get("text"))
    bridge = s45v71011_exact_topic_applicability_bridge(cluster_text)
    ...
    if not bridge.get("ok"):
        return {
            ...
            "evidence_url": None,
            "evidence_excerpt": None,
            "reason": "Exact topic was found, but no explicit General Annex applicability bridge was reproduced.",
            "attempts": attempts,
        }

    # Add any newly discovered General Annex URLs after the topic document has
    # been established.
    for d in s45v71011_collect_candidate_urls(fetched_docs):
        if s45v71011_is_general_annex_url(d) and d not in visited:
            fetched = s45v7107_fetch(d, timeout=45)
            if fetched.get("ok"):
                annex_docs.append(fetched)
                attempts.append({
                    "requested_url": d,
    ...
    for annex_doc in annex_docs:
        ...
        annex_excerpt = s45v71011_annex_excerpt(annex_text, task_key)
        if not annex_excerpt:
            continue

        cluster_url = normalize_text(cluster_doc.get("final_url"))

        combined = (
            "APPLICABILITY_BRIDGE:\n"
            + bridge.get("excerpt", "")[:5500]
            + "\n\nGENERAL_ANNEX_EVIDENCE:\n"
            + annex_excerpt[:7000]
        )
        ...
        return {
            ...
            "evidence_excerpt": combined[:12000],
            "evidence_reference": "EXACT_TOPIC_TO_GENERAL_ANNEX_B",
            "source_title": annex_doc.get("title") or "Horizon Europe General Annexes",
            "document_type": "OFFICIAL_EC_GENERAL_ANNEXES",
            "source_authority": "EUROPEAN_COMMISSION",
            "provenance_chain": [cluster_url, annex_url],
            "reason": (
                "v7.10.11 reproduced the exact-topic applicability bridge and "
                "the explicit substantive rule from freshly fetched official "
                "General Annexes."
            ),
            "attempts": attempts,
        }
    ...
    return {
        ...
        "evidence_excerpt": None,
        "reason": (
            "Exact-topic applicability was established, but no freshly fetched "
            "official General Annex document reproduced an explicit rule for this requirement."
        ),
        "attempts": attempts,
    }


def s45v71011_candidate_ready(result):
    if normalize_text(result.get("status")).upper() != "RESOLVED":
        return False
    if not (
        bool(result.get("exact_topic_verified"))
...
st.divider()
st.subheader("Stage 45 v7.10.11 — General Annex + Geographic Provenance Resolver")
st.info(
    "v7.10.11 păstrează TRL-ul existent și încearcă să rezolve Applicant, "
    "Consortium și Geographic exclusiv din documente oficiale EC proaspăt "
    "descărcate. General Annexes sunt acceptate numai după un bridge explicit "
    "din exact-topic Work Programme. Stage 46 rămâne gate-ul independent."
)

_v71011_tasks = [
    t for t in current_tasks
    if s45v71011_gate_task(t)
]

w1, w2, w3 = st.columns(3)
w1.metric("Gate requirements", len(_v71011_tasks))
w2.metric("Target handoff", len(_v71011_tasks))
w3.metric("Stage 46", "Independent re-fetch")

if st.button(
    "🧭 Run Stage 45 v7.10.11 provenance resolution",
    type="primary",
    use_container_width=True,
    key="stage45_v71011_run",
):
    _run_rows = (
        supabase.table("locked_evidence_worker_runs")
        .insert({
            ...
            "execution_run_id": execution_run_id,
            "opportunity_identity": identity,
            "total_tasks": len(_v71011_tasks),
            "worker_status": "RUNNING",
            "deep_resolution_version": "v7.10.11",
            "diagnostic_status": "CLEAN",
            "error_count": 0,
            "started_at": now_iso(),
            "summary": {
                "stage": 45,
                "version": "v7.10.11",
                "purpose": "GENERAL_ANNEX_AND_GEOGRAPHIC_PROVENANCE_RESOLUTION",
                "stage46_must_refetch": True,
                "fail_closed": True,
            },
            "updated_at": now_iso(),
    ...
    if not _run_rows:
        st.error("Nu am putut crea Stage 45 v7.10.11 run.")
    else:
        _run_id = str(_run_rows[0]["id"])
        _resolved = 0
        _waiting = 0
        ...
        for _idx, _task in enumerate(_v71011_tasks, 1):
            try:
                _result = s45v71011_discover_and_verify(_task)
                _ready = s45v71011_candidate_ready(_result)
                ...
                _audit.append({
                    "Requirement": _task.get("requirement_label"),
                    "Requirement key": s45v71011_requirement_key(_task),
                    "Status": _status,
                    "Evidence reference": _result.get("evidence_reference"),
                    "Evidence URL": _result.get("evidence_url"),
                    "Provenance chain": _result.get("provenance_chain"),
            ...
            _bar.progress(_idx / max(1, len(_v71011_tasks)))

        _final = (
            "FAILED" if _failed and not _resolved and not _waiting
            else "PARTIAL_FAILURE" if _failed
            else "COMPLETED" if _resolved == len(_v71011_tasks)
            else "WAITING"
        )
        ...
        st.session_state["stage45_v71011_results"] = _audit
        st.success(
            f"Stage 45 v7.10.11: {_final} — handoff ready {_resolved}, "
            f"waiting {_waiting}, failed {_failed}."
        )
        st.rerun()
...
_v71011_runs = [
    r for r in _v71011_runs
    if normalize_text(r.get("deep_resolution_version")).lower() == "v7.10.11"
]

if _v71011_runs:
    _latest_v71011 = _v71011_runs[0]
    _sum = _latest_v71011.get("provenance_summary") or {}
    if not isinstance(_sum, dict):
        _sum = {}

    st.subheader("Latest Stage 45 v7.10.11 Result")
    y1, y2, y3, y4 = st.columns(4)
    y1.metric("Status", _latest_v71011.get("worker_status") or "—")
    y2.metric("Handoff ready", _latest_v71011.get("official_tasks_resolved") or 0)
    y3.metric("Waiting", _latest_v71011.get("official_tasks_waiting") or 0)
    y4.metric("Failed", _latest_v71011.get("failed_tasks") or 0)

    if _sum.get("audit"):
        st.dataframe(_sum["audit"], use_container_width=True, hide_index=True)

st.caption(
    "v7.10.11 invariant: General Annex evidence is accepted only after exact-topic "
    "applicability is explicitly reproduced. Geographic eligibility is never inferred "
    "from project location or identity. Stage 46 must independently re-fetch all evidence."
)
