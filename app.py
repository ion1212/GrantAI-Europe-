...
def require_login() -> None:
    if auth_user_id():
        return
    st.title("🇪🇺 GrantAI Europe — Etapa 8 FIX")
...
st.title("🇪🇺 GrantAI Europe — Etapa 8 FIX")
st.caption("Funding Opportunity Engine — parser reparat")
...
with engine_tab:
    ...
    if diagnostics:
        with st.expander("Diagnostic API"):
            ...
            d3.metric("Cu titlu", diagnostics["with_title"])
            d4.metric("Cu deadline", diagnostics["with_deadline"])
            st.write(f'**Cu descriere:** {diagnostics["with_description"]}')
